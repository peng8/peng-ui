...
<template>
  <NuxtLink
    :to="localePath(`/products/categories/${item.slug}`)"
  ...
  >
    ...
    <div class="relative aspect-[4/3] overflow-hidden bg-mist-dark">
      <UiLazyImage :src="item.image" :alt="isZh ? item.nameZh : item.name" ratio="aspect-[4/3]" class="h-full" />
    ...
    <div class="flex flex-1 flex-col p-6">
      ...
      <div class="mt-4 flex items-center justify-between border-t border-mist pt-4">
        ...
        <span class="text-sm font-semibold text-navy-500">{{ isZh ? '500 瓶' : item.moq }}</span>
      </div>
    </div>
  </NuxtLink>
</template>
